...
from app import db
from models.skill_require import SkillRequire
from utils import commons
from utils.response_code import RET, error_map_EN
from utils.loggings import loggings


class SkillRequireController(SkillRequire):
    ...
    def get(cls, **kwargs):
        try:
            ...
            skill_require_info = db.session.query(cls).filter(*filter_list)
            
            count = skill_require_info.count()
            pages = math.ceil(count / size)
            skill_require_info = skill_require_info.limit(size).offset((page - 1) * size).all()
   
            results = commons.query_to_dict(skill_require_info)
    ...
    def add_list(cls, **kwargs):
        param_list = kwargs.get('SkillRequireList')
        model_list = []
